...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < num_tracefiles; i++) {
    ...
    num_valid_libc += libc_stats[i].valid;
    if (check_heap) {
  ...
  if (autograder) {
    printf(
        "-------------------- Heap Validator Results --------------------\n");
    printf("libc malloc Num valid: %d, out of: %d trace files.\n",
           num_valid_libc, num_tracefiles);
  }
  ...
  if (run_bad) {
    ...
    for (i = 0; i < num_tracefiles; i++) {
      ...
      num_valid_bad += bad_stats[i].valid;
      if (check_heap) {
    ...
    if (autograder) {
      printf(
          "-------------------- Heap Validator Results --------------------\n");
      printf("bad malloc Num valid: %d, out of: %d trace files.\n",
             num_valid_bad, num_tracefiles);
    }
  ...
  for (i = 0; i < num_tracefiles; i++) {
    ...
    num_valid_student += mm_stats[i].valid;
    if (check_heap) {
  ...
  if (autograder) {
    printf(
        "-------------------- Heap Validator Results --------------------\n");
    printf("student malloc Num valid: %d, out of: %d trace files.\n",
           num_valid_student, num_tracefiles);
    printf("perfidx: %f\n", perfindex);
  }
