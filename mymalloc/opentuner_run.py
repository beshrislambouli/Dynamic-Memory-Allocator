...
class MdriverTuner(MeasurementInterface):
  ...
  def run(self, desired_result, input, limit):
    ...
    if awsrun:
        ...
        student_malloc_num_valid_str = result.get('student malloc Num valid', correct)
        num_correct = int(re.search(r'\d+', student_malloc_num_valid_str).group())
        if num_correct != NUM_TRACE_FILES:
            print("run not correct. Expected: {0} valid trace files but got: {1}".format(NUM_TRACE_FILES, num_correct))
            return Result(accuracy=accuracy, time=time)
